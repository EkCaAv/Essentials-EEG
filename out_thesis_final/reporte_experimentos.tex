...
\begin{table}[H]
\centering
\caption{Sujetos incluidos en el experimento}
\label{tab:sujetos}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{llrrr}
\toprule
\textbf{ID sujeto} & \textbf{Archivos EDF} & \textbf{Ventanas ictales (est.)} &
\textbf{Ventanas interictales (est.)} \\
\midrule
chb05 & 39 & 30  & 7\,892 \\
chb09 & 19 & 111 & 9\,300 \\
chb14 & 26 & 131 & 8\,695 \\
...
chb23 & 9  & 180 & 2\,700 \\
\midrule
\textbf{Total} & \textbf{172} & \textbf{703} & \textbf{47\,787} \\
\bottomrule
\end{tabular}
\end{table}
...
global ictal/interictal. Derecha: recuento estimado de ventanas por sujeto.}
...
el criterio de éxito mínimo (0.15). Solo Random Forest con B3 y B4 y Gradient Boosting
con B3 superan ese umbral.}
...
Este test no paramétrico es apropiado dado el tamaño reducido de la muestra ($n = 7$ folds)
y la no normalidad esperada de las métricas.

\textbf{Advertencia de potencia:} con $n=7$ y $\alpha=0.05$, la potencia del test para
efectos medianos es aproximadamente 0.35 \cite{cohen1988}. Por tanto, los resultados no
significativos no demuestran equivalencia, sino insuficiencia de evidencia.
...
Celdas rojizas indican pares con menor p-valor; el asterisco (*) marca $p < 0.05$.
Solo el par Gradient Boosting vs SVM muestra diferencia significativa en el feature set
\texttt{bp\_plus\_rms} ($p=0.040$).}
\label{fig:wilcoxon}
\end{figure}
...
\toprule
\textbf{Par de modelos} & \textbf{W} & \textbf{p-valor} & \textbf{Interpretación} \\
\midrule
Gradient Boosting vs Random Forest & 141.0 & 0.811 & n.s. — sin evidencia de diferencia \\
Gradient Boosting vs Reg. Logística & 117.0 & 0.360 & n.s. — sin evidencia de diferencia \\
\rowcolor{yellow!25}
\textbf{Gradient Boosting vs SVM} & \textbf{78.0} & \textbf{0.040*} &
  \textbf{Evidencia probable de diferencia. GB $>$ SVM en PR-AUC.} \\
Random Forest vs Reg. Logística & 101.0 & 0.169 & n.s. — sin evidencia de diferencia \\
Random Forest vs SVM & 84.0 & 0.060 & n.s. (marginal) \\
Reg. Logística vs SVM & 146.0 & 0.922 & n.s. — sin evidencia de diferencia \\
\bottomrule
\multicolumn{4}{l}{\small * $p < 0.05$; interpretación epistemológica según DOE §5.}
\end{tabular}
\end{table}
...
\item \textbf{Las características estadísticas de orden superior aportan valor.}
  La adición de kurtosis y skewness al feature set mejora el PR-AUC de Random Forest
  y Gradient Boosting en un 50--70\,\% respecto al baseline de bandpower puro.
...
\item \textbf{Solo Gradient Boosting vs SVM muestra diferencia estadísticamente
  significativa} ($p=0.040$) en el feature set de referencia. Los demás pares no
  alcanzan significancia, aunque esto es atribuible principalmente a la baja potencia
  del test con $n=7$ folds, y no a equivalencia real entre modelos.
